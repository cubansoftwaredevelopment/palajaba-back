...
def _display_category_name(category_id: str) -> str:
    return business_category_name(category_id)
...
def _normalize_marketplace_category_id(category_id: str) -> str:
    ...
    if normalized in KNOWN_BUSINESS_CATEGORY_IDS or normalized in KNOWN_PRODUCT_CATEGORY_IDS:
        return normalize_business_category_id(normalized)
...
def _build_marketplace_products_query(
    ...
    *,
    ...
    if global_category_id:
        canonical_id = normalize_business_category_id(global_category_id)
        source_ids = business_category_source_ids(canonical_id)
...
async def _aggregate_category_stats(
    ...
    *,
    ...
    for item in results:
        raw_id = str(item["_id"])
        canonical_id = normalize_business_category_id(raw_id)
        bucket = merged.setdefault(canonical_id, {"count": 0, "popularity": 0})
